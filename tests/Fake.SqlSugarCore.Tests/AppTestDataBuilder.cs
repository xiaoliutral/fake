...
public class AppTestDataBuilder
{
    ...
    public AppTestDataBuilder(IOrderRepository orderRepository, IRepository<Buyer> buyerRepository)
    {
    ...
    public virtual async Task BuildAsync()
    {
        ...
        await db.InsertNav(order).Include(x => x.OrderItems).ExecuteCommandAsync();
    }
