...
{
    public ConsulWeightServiceBalancer(ILogger<ConsulWeightServiceBalancer> logger, IConsulClient client) : base(logger,
        client)
    {
    }
    ...
    protected override void OnServiceEntry(string serviceName, List<ServiceEntry> entries)
    {
        var weights = entries.Select(x =>
                int.TryParse(x.Service.Meta[nameof(FakeConsulRegisterOptions.Weight)], out var weight) ? weight : 0)
            .ToList();
