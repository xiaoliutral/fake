...
public class FakeGrpcResolverFactory : ResolverFactory
{
    ...
    public FakeGrpcResolverFactory(IBackoffPolicyFactory backoffPolicyFactory, IServiceBalancer serviceBalancer,
        IOptions<FakeGrpcClientOptions> options)
    {
    ...
    {
        return new FakeGrpcResolver(options.LoggerFactory, _backoffPolicyFactory, options.Address, _serviceBalancer,
            _options);
    }
}