...
) : IApplicationModelConvention
{
    ...
    protected virtual void ConfigureSelector(ControllerModel controller)
    {
        ...
        foreach (var action in controller.Actions)
        {
            ...
            var fakeSelectorModel = new SelectorModel
            {
                AttributeRouteModel = routeMode,
                ActionConstraints = { new HttpMethodActionConstraint(new[] { httpVerb }) }
            };
            
            // 一个aspnetcore的selector都没有时直接用fake约定的
            if (!action.Selectors.Any())
            {
                action.Selectors.Add(fakeSelectorModel);
            }
            else
            {
                // normal selector
                foreach (var selector in action.Selectors)
                {
                    ...
                    {
                        selector.ActionConstraints.Add(new HttpMethodActionConstraint([httpVerb]));
                    }
                }
            }
        }
